...
void main() {
  group('HubProvisioningBloc', () {
    ...
    test('emits permissionRequired when bluetooth permissions are denied',
      ...
      await _waitUntil(() {
        return bloc.state.phase == HubProvisioningPhase.permissionRequired;
      });
    ...
    test('emits selectDevice with empty candidates when no CAM device is found',
      ...
      await _waitUntil(() {
        return bloc.state.phase == HubProvisioningPhase.selectDevice;
      });
    ...
    test('emits failure when Wi-Fi scan fails for a selected device', () async {
      blePermissionService.checkStatusValue =
          BlePermissionRequirementStatus.granted;
      bleProvisioningService.scanDevicesResult = const [
    ...
    test(
        'supports provisioning success followed by retry sync without resending Wi-Fi',
        () async {
      blePermissionService.checkStatusValue =
          BlePermissionRequirementStatus.granted;
      bleProvisioningService.scanDevicesResult = const [
      ...
      repository.upsertResults = [
        Right(_binding(status: SpaceHubBindingStatus.syncPending)),
        Right(_binding()),
      ];
    ...
    test('allows reconfiguration scan from an existing binding', () async {
      ...
      bloc.add(const HubProvisioningBleScanRequested());
      await _waitUntil(
          () => bloc.state.phase == HubProvisioningPhase.selectDevice);

      expect(bloc.state.bleCandidates, hasLength(1));
      expect(bloc.state.binding?.bleDeviceName, 'CAM-ESP32-01');
    });
